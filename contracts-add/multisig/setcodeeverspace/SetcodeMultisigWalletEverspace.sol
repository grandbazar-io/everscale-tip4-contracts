...
contract SetcodeMultisigWalletEverspace is IAccept {
    ...
    */
    ...
    function confirmTransaction(uint64 transactionId) public {
        ...
        _removeExpiredTransactions();
        optional(Transaction) optTxn = m_transactions.fetch(transactionId);        
        require(optTxn.hasValue(), 102);
    ...
    function _removeExpiredTransactions() inline private {
        ...
        optional(uint64, Transaction) minTxn = m_transactions.min();
        require(minTxn.hasValue(), 102);
        
        (uint64 trId, Transaction txn) = minTxn.get();
    ...
    function getTransactions() public view returns (Transaction[] transactions) {
        ...
        optional(uint64, Transaction) optTxn = m_transactions.min();
        while (optTxn.hasValue()) {
            // returns only not expired transactions
            (uint64 id, Transaction txn) = optTxn.get();
            if (id > bound) {
                transactions.push(txn);
            }
            optTxn = m_transactions.next(id);
        }
    }
    ...
    function getTransactionIds() public view returns (uint64[] ids) {
        ...
        while (optTxn.hasValue()) {
            (trId, ) = optTxn.get();
            ids.push(trId);
            optTxn = m_transactions.next(trId);
        }
    }
    ...
    function getCustodians() public view returns (CustodianInfo[] custodians) {
        optional(uint256, uint8) optTxn = m_custodians.min();
        while (optTxn.hasValue()) {
            (uint256 key, uint8 index) = optTxn.get();
            custodians.push(CustodianInfo(index, key));
            optTxn = m_custodians.next(key);
        }
    }    
    ...
    function confirmUpdate(uint64 updateId) public {
        uint8 index = _findCustodian(msg.pubkey());
        _removeExpiredUpdateRequests();
        optional(UpdateRequest) optRequest = m_updateRequests.fetch(updateId);
        require(optRequest.hasValue(), 115);
        UpdateRequest request = optRequest.get();
    ...
    function getUpdateRequests() public view returns (UpdateRequest[] updates) {
        uint64 bound = _getExpirationBound();
        optional(uint64, UpdateRequest) optReq = m_updateRequests.min();
        while (optReq.hasValue()) {
            (uint64 updateId, UpdateRequest req) = optReq.get();
            if (updateId > bound) {
                updates.push(req);
            }
            optReq = m_updateRequests.next(updateId);
        }
    }
    ...
    function _removeExpiredUpdateRequests() inline private {
        ...
        while (needCleanup) {
            ...
            optReq = m_updateRequests.next(updateId);
            if (!optReq.hasValue()) { break; }
            (updateId, req) = optReq.get();
